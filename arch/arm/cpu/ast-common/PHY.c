...
//			phy_basic_setting( eng );
		...
		// Check register 0x11 bit2 Link OK or not OK
		while ( ( (phy_read( eng, 17 ) & 0x0400) != 0x0400 ) &&
			( wait_phy_ready++ < 50 ) )
			phy_delay(100);
		if ( wait_phy_ready >= 50 ) {
			printf("Timeout: init RTL8211E\n");
			PRINTF( FP_LOG, "Timeout: init RTL8211E\n");
		}
	}
...
//		phy_delay(400);
		...
		while ( ( (phy_read( eng, 1 ) & 0x0004) != 0x0004 ) &&
			( wait_phy_ready++ < 50 ) )
			phy_delay(100);
		if ( wait_phy_ready >= 50 ) {
			printf("Timeout: init RTL8201F\n");
			PRINTF( FP_LOG, "Timeout: init RTL8201F\n");
		}
	}
}
...
//		}
		...
		if ( wait_phy_ready >= 50 ) {
			...
			PRINTF( FP_LOG, "Timeout: init RTL8211F\n");
		}
		phy_write( eng, 31, 0x0000 );
	}
}
...
void phy_realtek6 (MAC_ENGINE *eng) {//RTL8363S
	...
	if ( eng->run.TM_Burst ) {
        printf("The mode doesn't support in RTL8363S.\n");
	}
	else if ( eng->phy.loop_phy ) {
...
void phy_sel (MAC_ENGINE *eng) {
	...
	else if ( phy_chk( eng, 0x001c, 0xc930, 0xffff      ) ) { sprintf( eng->phy.PHYName, "RTL8363S"         ); if ( eng->arg.GEn_InitPHY ) phy_realtek6 ( eng );}//RTL8363S         1G/100/10M  RGMII Switch
...
void recov_phy (MAC_ENGINE *eng) {
	...
	if      ( phy_chk( eng, 0x0362, 0x5e6a, 0xfff0      ) ) recov_phy_broadcom0( eng );//BCM54612
	else if ( phy_chk( eng, 0x0362, 0x5d10, 0xfff0      ) ) recov_phy_broadcom0( eng );//BCM54616S
	else if ( phy_chk( eng, 0x0143, 0xBCA0, 0xfff0      ) ) recov_phy_broadcom0( eng );//BCM5481
	else if ( phy_chk( eng, 0x0020, 0x60B0, 0xfff0      ) ) recov_phy_broadcom0( eng );//BCM5464SR
	else if ( phy_chk( eng, 0x0141, 0x0dd0, 0xfff0      ) ) recov_phy_marvell2 ( eng );//88E1512
	else if ( phy_chk( eng, 0xff00, 0x1761, 0xffff      ) ) recov_phy_marvell1 ( eng );//88E6176
	else if ( phy_chk( eng, 0x0141, 0x0e90, 0xfff0      ) ) recov_phy_marvell0 ( eng );//88E1310
	else if ( phy_chk( eng, 0x0141, 0x0cc0, PHYID3_Mask ) ) recov_phy_marvell  ( eng );//88E1111
	else if ( phy_chk( eng, 0x001c, 0xc916, 0xffff      ) ) recov_phy_realtek5 ( eng );//RTL8211F
	else if ( phy_chk( eng, 0x001c, 0xc915, 0xffff      ) ) recov_phy_realtek2 ( eng );//RTL8211E
	else if ( phy_chk( eng, 0x001c, 0xc914, 0xffff      ) ) recov_phy_realtek1 ( eng );//RTL8211D
	else if ( phy_chk( eng, 0x001c, 0xc912, 0xffff      ) ) recov_phy_realtek3 ( eng );//RTL8211C
	else if ( phy_chk( eng, 0x0007, 0x0421, 0xfff0      ) ) recov_phy_vitesse  ( eng );//VSC8601
}
